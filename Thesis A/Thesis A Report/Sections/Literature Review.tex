...
Natural convection is the process by which heat transferred to a fluid raises its temperature, reducing its density and generating buoyant forces. These forces cause the warmer, lighter fluid to rise, carrying the absorbed heat to another location where it can be dissipated. \cite{Zohuri2020ThermosyphonApplications} An example of natural convection is shown in Figure \ref{fig:cooling_of_boiled_egg_natural_convection}.

\begin{figure}[ht]
    % \captionsetup{aboveskip=5pt, belowskip=0pt}
    \centering
    \includegraphics[width=0.5\textwidth]{Figures/cooling_of_boiled_egg_natural_convection.pdf}
...
% Natural Convection Velocity Boundary Layer


% Natural Convection Thermal Boundary Layer


% How do boundary layers affect convective heat transfer rate


% RCM: Tilt angle optimization
A 2021 study led by A. Abdulmunem from the University of Malaya, Malaysia, investigated the impact of tilt angle on the performance of photovoltaic systems. Abdulmunem et al. observed that as the tilt angle increased from $0^\circ$ (horizontal) to $90^\circ$ (vertical), the temperature reduction in the photovoltaic cell increased from $-0.4\%$ at $0^\circ$ to $-12\%$ at $90^\circ$. This trend was attributed to enhanced convective heat transfer. Consequently, Abdulmunem et al. concluded that increasing the tilt angle of the photovoltaic module away from the horizontal axis and towards the vertical axis improves the rate of convective heat transfer. \cite{Abdulmunem2021NumericalSink}\vspace{0.5em}

Abdulmunem's findings are supported by theoretical calculations as well.\par
The Rayleigh number, Ra, which can be calculated using Equation \ref{eq:rayleigh_number_function}, is the ratio of buoyancy forces and (the products of) thermal and momentum diffusivities.\par
\begin{equation}
    \text{Ra}_L = \frac{g\beta(T_s-T_\infty)L_c^3}{\nu\alpha}
    \label{eq:rayleigh_number_function}
\end{equation}

For an inclined photovoltaic module between $0^\circ$ and $60^\circ$, the gravitational force, $g$, is replaced with $g\,\cos(\theta)$, where $\theta$ represents the angle between the vertical axis and the photovoltaic module, shown in Figure \ref{fig:inclined_plate}.
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.5\linewidth, trim=0 20 0 20, clip]{Figures/inclined_plate.pdf}
    \caption{Inclined photovoltaic module. \cite{Cengel2014NaturalConvection}}
    \label{fig:inclined_plate}
\end{figure}

Thus, as the photovoltaic module 

For simple empirical correlations, the average Nusselt number, Nu, can be calculated using Equation \ref{eq:avg_nusselt_number_natural_convection}.
\begin{equation}
    \text{Nu} = \frac{hL_c}{k} = C(\text{Gr}_L\text{Pr})^n = C\text{Ra}_L^n
...
\begin{itemize}
    \item Show the natural convection currents that occur for a horizontal hot plate.
    \item Show the natural convection currents that occur for a hot plate at angle, $theta$.
    \item Explain how the angle affects the natural convective heat transfer rate.
    \item Cite a study that determined the ideal angle for free convective heat transfer.
\end{itemize}

- The angle of tilt of a photovoltaic module significantly affects natural convective heat transfer.\par
- Lower tilt angles observe weaker natural convection because the boundary layer of hot air tends to stay on the surface longer.\par
- Furthermore, there is less air circulation which means that less heat is being removed so the panel can get hotter.\par

- With steeper tilt angles, there is stronger bouyancy-driven flow.\par
- Hot air rises more freely along the panel's surface.\par
- Enhanced airflow leads to more effective cooling through convection.\par

- Mathematically, an increase in $Nu$ due to tilt, facilitates better heat transfer, shown through an increase in the convective heat transfer coefficient.\par

- Most photovoltaic modules are positioned at an angle of about [INSERT ANGLE HERE] due to them being mainly placed on roofs which have an angle.\par

\begin{equation}
    h = \frac{Nu \times k}{L}
    \label{convective_heat_transfer_coefficient}
\end{equation}


\textbf{When to consider natural convection?}

% RCM: Heat Sink

% RCM: Phase Change Materials with Convection

% A comparison of natural convection and forced convection - why forced convection is better for photovoltaic modules
\pagebreak
