...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.5\textwidth, trim=0 10 0 10, clip]{Figures/cooling_of_boiled_egg_natural_convection.pdf}
...
The Rayleigh number for a tilted photovoltaic module, $\text{Ra}_\theta$, is shown in Equation \ref{eq:rayleigh_number_tilted_function} \cite{Cengel2014NaturalConvection}.
...
Finally, an inspection of Equation \ref{eq:rate_of_convective_heat_transfer_function} shows that an increase in the convective heat transfer coefficient, $h$, results in a higher natural convective heat transfer rate, $\dot{Q}_\text{conv}$, thereby supporting Abdulmunem et al.'s experimental findings.\vspace{0.5em}
...
Natural convection-based cooling methods can be effective tools for reducing an object's temperature. In the context of photovoltaic modules, however, forced convection has been shown to be significantly more efficient at facilitating heat transfer from the module to its surroundings. A 2021 study led by A. Almuwailhi from King Saud University investigated the impact of both natural and forced convection cooling methods on the efficiency of photovoltaic modules. To promote natural convective heat transfer, Almuwailhi's team experimented with different channel air gaps behind the module. The most promising configuration featured an air gap of $120$ mm, which led to a $1.2\%$ increase in photovoltaic module efficiency. For forced convection, the researchers used airflow velocities of $2$ m/s and $3$ m/s. The results demonstrated a $3.8\%$ increase in efficiency with $2$ m/s airflow, and a $4\%$ increase with $3$ m/s airflow. \cite{Almuwailhi2023InvestigatingRiyadh} Therefore, this thesis will focus on forced convection based cooling methods.

\subsubsubsection{Forced Convection and Relevant Cooling Methods}
...
The convective heat transfer coefficient, $h$, is inversely proportional to the thermal boundary layer thickness, $\delta_t$. Forced convection cooling techniques enhance heat transfer by reducing the temperature gradient distance between the surface temperature, $T_s$, and the ambient temperature, $T_\infty$, effectively thinning the boundary layer. Thus, forced convection cooling methods significantly improve convective heat transfer performance.\vspace{0.5em}

% RCM: Air
In 2024, a study led by I. Al-Masalha from AlBalqa Applied University focused on improving the efficiency of photovoltaic modules using air fans, water sprinklers, and combined cooling systems. One of the methods explored involved using air fans to direct airflow over the surface of the modules. As a result, the temperature of the modules decreased by $6\%$ to $26\%$, as shown in Figure \ref{fig:air_cooling_temperature_graph}. This reduction in temperature led to an efficiency increase of $0.44\%$ to $1.45\%$, illustrated in Figure \ref{fig:air_cooling_efficiency_graph}. \cite{Al-Masalha2024ImprovingSystems}
...
% RCM: Water
Al-Masalha et al.'s water cooling method involved spraying water directly onto the photovoltaic modules to dissipate heat. As illustrated in Figure \ref{fig:water_cooling_temperature_graph}, this technique resulted in a temperature reduction of the photovoltaic module by $20.45\%$ to $35.44\%$. Consequently, as shown in Figure \ref{fig:water_cooling_efficiency_graph}, the temperature reduction led to an efficiency improvement ranging from $0.6\%$ to $1.8\%$. \cite{Al-Masalha2024ImprovingSystems}
...
% RCM: Combined Cooling System
 Finally, Al-Masalha's team combined water spraying and air cooling techniques to develop a hybrid cooling method. This system achieved the most effective results in terms of both temperature reduction and efficiency. As shown in Figure \ref{fig:hybrid_cooling_temperature_graph}, hybrid cooling led to a temperature decrease ranging from $30\%$ to $40\%$. This substantial reduction in temperature improved the photovoltaic module’s efficiency by $0.88\%$ to $2\%$. \cite{Al-Masalha2024ImprovingSystems}
