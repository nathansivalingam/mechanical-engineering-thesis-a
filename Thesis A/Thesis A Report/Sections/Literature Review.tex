...
Convection is the mode of energy transfer between a solid surface and the adjacent liquid or gas that is in motion. The formula commonly used to calculate the rate of heat convective heat transfer can be expressed by Newton's law of cooling, shown in Equation \ref{eq:rate_of_convective_heat_transfer_function}. \cite{Cengel2014IntroductionConcepts}
...
\textbf{What factors affect convection (factors that influence the variables in the equation)?}\par

Figure \ref{fig:convective_heat_transfer_diagram} illustrates the velocity and temperature profiles associated the photovoltaic module due to convective heat transfer.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.75\linewidth]{Figures/convective_heat_transfer_diagram.pdf}
    \caption{Heat transfer from the photovoltaic module to air by convection. Adapted from \cite{Cengel2014IntroductionConcepts}}
    \label{fig:convective_heat_transfer_diagram}
\end{figure}

How is convection experienced on photovoltaic modules (Charitha's Explanation)?\par
- Velocity Boundary Layers\par
- Thermal Boundary Layers\vspace{0.5em}
...
\subsubsubsection{Natural Convection and the Relevant Cooling Methods}
...
\subsubsubsection{Forced Convection and the Relevant Cooling Methods}
Hello World\par
