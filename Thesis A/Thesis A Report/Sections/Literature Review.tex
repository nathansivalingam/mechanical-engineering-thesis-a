...
Figure \ref{fig:vortex_generator} also illustrates two key features associated with vortex generators: the forward stagnation point and the separation point. The forward stagnation point denotes the location where the fluid velocity, $u$, is zero, while the separation point marks where the airflow detaches from the object's surface.\par
At the point of flow separation, vortices, known as Kármán vortices in the context of cylinders, are formed.

- At the separation point, strong vortices are created.
- If the incoming boundary layer is turbulent, flow separation around the cylinder is smaller and more controlled, producing stronger, more coherent vortices.
- These vortices mix high-momentum outer fluid with low-momentum near-wall fluid, increasing momentum and energy exchange near the surface.
- Mixing thins the thermal boundary layer, improving convective heat transfer.
